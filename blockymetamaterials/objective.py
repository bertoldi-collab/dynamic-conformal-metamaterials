...
def compute_xcorr2d(signal0: jnp.ndarray, signal1: jnp.ndarray, shift: Tuple[Optional[int], Optional[int]] = (None, None)):
    ...
    xcorr2d = jsp.signal.correlate2d(signal0, signal1) / jsp.signal.correlate2d(signal0, signal0).max()

    if shift == (None, None):
        return xcorr2d
    elif shift[1] is None and shift[0] is not None:
        return xcorr2d[signal1.shape[0] - 1 + shift[0], :]  # Slice along 1 axis
    elif shift[0] is None and shift[1] is not None:
        return xcorr2d[:, signal1.shape[1] - 1 + shift[1]]  # Slice along 0 axis
...
def compute_xcorr(signal0: jnp.ndarray, signal1: jnp.ndarray, shift: Optional[int] = None):
    ...
    xcorr = jsp.signal.correlate(signal0, signal1) / jsp.signal.correlate(signal0, signal0).max()
...
def compute_max_xcorr2d_at_shift(signal0: jnp.ndarray, signal1: jnp.ndarray, shift: int, shift_axis: Literal[0, 1] = 0):
    ...
    xcorr2d_slice = compute_xcorr2d(signal0, signal1, shift=(shift, None) if shift_axis == 0 else (None, shift))

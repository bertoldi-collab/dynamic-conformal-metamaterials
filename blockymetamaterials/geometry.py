...
class RotatedSquareGeometry(LatticeGeometry):
    ...
    def bond_position_to_id(self, bond_2d_id: Tuple[int, int], bond_type: Literal["horizontal", "vertical"]) -> int:
        """
        Convert a 2D bond position to a bond id.

        Args:
            bond_2d_id (Tuple[int, int]): 2D bond position (i, j) where i is the horizontal index and j is the vertical index.
            bond_type (Literal["horizontal", "vertical"]): Type of bond.
        """
        bond_2d_id = jnp.array(bond_2d_id)
        if bond_type == "horizontal":
            # Make sure bond_2d_id is within the bounds
            bond_2d_id = jnp.clip(bond_2d_id, 0, jnp.array([self.n1_blocks-1-1, self.n2_blocks-1]))
            bond_id = (lambda bond_2d_id: bond_2d_id[0] + bond_2d_id[1]*(self.n1_blocks-1))(bond_2d_id)
        elif bond_type == "vertical":
            # Make sure bond_2d_id is within the bounds
            bond_2d_id = jnp.clip(bond_2d_id, 0, jnp.array([self.n1_blocks-1, self.n2_blocks-1-1]))
            bond_id = (lambda bond_2d_id: bond_2d_id[0] + bond_2d_id[1] *
                       self.n1_blocks + (self.n1_blocks-1)*self.n2_blocks)(bond_2d_id)
        else:
            raise ValueError("Invalid bond type. bond_type must be either 'horizontal' or 'vertical'.")
        return bond_id
...
class QuadGeometry(LatticeGeometry):
    ...
    def compute_shifts_mask(
            self,
            bond_2d_id: jnp.ndarray,
            vertical_bond_patch_size: jnp.ndarray,
            horizontal_bond_patch_size: jnp.ndarray,
            bond_type: Union[Literal["vertical", "horizontal"], List[Literal["vertical", "horizontal"]]],):
        """
        Compute a mask that selects the shifts that are in the patch of the bond_2d_id or array of bond_2d_id.

        Args:
            bond_2d_id (jnp.ndarray): 2D position of the bond in the grid. Shape (2,) or (n_bonds, 2).
            vertical_bond_patch_size (jnp.ndarray): Size of the patch for vertical bonds. Shape (2,) or (n_bonds, 2).
            horizontal_bond_patch_size (jnp.ndarray): Size of the patch for horizontal bonds. Shape (2,) or (n_bonds, 2).
            bond_type (Union[Literal["vertical", "horizontal"], List[Literal["vertical", "horizontal"]]]): Type of the bond. Shape (,) or (n_bonds,).

        Returns:
            Tuple[jnp.ndarray, jnp.ndarray]: Tuple of masks for horizontal and vertical shifts.
        """
        horizontal_shifts_mask = jnp.zeros((self.n1_blocks+1, self.n2_blocks), dtype=jnp.bool_)
        vertical_shifts_mask = jnp.zeros((self.n1_blocks, self.n2_blocks+1), dtype=jnp.bool_)
        # If bond_type is a list, we loop over to create a global mask. All other parameters should be lists/arrays of the same length.
        if type(bond_type) == str:
            bond_2d_id = [bond_2d_id] if len(bond_2d_id.shape) == 1 else bond_2d_id
            vertical_bond_patch_size = [vertical_bond_patch_size]*len(bond_2d_id) if len(
                vertical_bond_patch_size.shape) == 1 else vertical_bond_patch_size
            horizontal_bond_patch_size = [horizontal_bond_patch_size]*len(bond_2d_id) if len(
                horizontal_bond_patch_size.shape) == 1 else horizontal_bond_patch_size
            bond_type = [bond_type]*len(bond_2d_id)

        for _bond_2d_id, _vertical_bond_patch_size, _horizontal_bond_patch_size, _bond_type in zip(bond_2d_id, vertical_bond_patch_size, horizontal_bond_patch_size, bond_type):
            if _bond_type == "vertical":
                horizontal_range = jnp.clip(
                    _bond_2d_id[0] + jnp.array([-_horizontal_bond_patch_size[0]//2+1, _horizontal_bond_patch_size[0]//2+1]), 0, self.n1_blocks+1)
                vertical_range = jnp.clip(
                    _bond_2d_id[1] + jnp.array([-_horizontal_bond_patch_size[1]//2+1, _horizontal_bond_patch_size[1]//2+1]), 0, self.n2_blocks)
                horizontal_shifts_mask = horizontal_shifts_mask.at[
                    horizontal_range[0]:horizontal_range[1],
                    vertical_range[0]:vertical_range[1],
                ].set(True)
                horizontal_range = jnp.clip(
                    _bond_2d_id[0] + jnp.array([-_vertical_bond_patch_size[0]//2+1, _vertical_bond_patch_size[0]//2+1]), 0, self.n1_blocks)
                vertical_range = jnp.clip(
                    _bond_2d_id[1]+1 + jnp.array([-_vertical_bond_patch_size[1]//2+1, _vertical_bond_patch_size[1]//2+1]), 0, self.n2_blocks+1)
                vertical_shifts_mask = vertical_shifts_mask.at[
                    horizontal_range[0]:horizontal_range[1],
                    vertical_range[0]:vertical_range[1],
                ].set(True)
            elif _bond_type == "horizontal":
                horizontal_range = jnp.clip(
                    _bond_2d_id[0]+1 + jnp.array([-_horizontal_bond_patch_size[0]//2+1, _horizontal_bond_patch_size[0]//2+1]), 0, self.n1_blocks+1)
                vertical_range = jnp.clip(
                    _bond_2d_id[1] + jnp.array([-_horizontal_bond_patch_size[1]//2+1, _horizontal_bond_patch_size[1]//2+1]), 0, self.n2_blocks)
                horizontal_shifts_mask = horizontal_shifts_mask.at[
                    horizontal_range[0]:horizontal_range[1],
                ...
                horizontal_range = jnp.clip(
                    _bond_2d_id[0] + jnp.array([-_vertical_bond_patch_size[0]//2+1, _vertical_bond_patch_size[0]//2+1]), 0, self.n1_blocks)
                vertical_range = jnp.clip(
                    _bond_2d_id[1] + jnp.array([-_vertical_bond_patch_size[1]//2+1, _vertical_bond_patch_size[1]//2+1]), 0, self.n2_blocks+1)
                vertical_shifts_mask = vertical_shifts_mask.at[
                    horizontal_range[0]:horizontal_range[1],
                    vertical_range[0]:vertical_range[1],
                ].set(True)

        return horizontal_shifts_mask, vertical_shifts_mask

    def compute_constrained_geometry_bond_patches(
            self,
            default_shifts: Tuple[jnp.ndarray, jnp.ndarray],
            bond_2d_id: jnp.ndarray,
            vertical_bond_patch_size: jnp.ndarray,
            horizontal_bond_patch_size: jnp.ndarray,
            bond_type: Union[Literal["vertical", "horizontal"], List[Literal["vertical", "horizontal"]]],):
        """
        Setup the constrained geometry parametrized by patches of shifts around the selected bond_2d_id. Make sure to call self.compute_geometry before.
        This defines the following functions:
        - block_centroids_constrained: (designed_shifts) -> block_centroids
        - centroid_node_vectors_constrained: (designed_shifts) -> centroid_node_vectors
        - reference_bond_vectors_constrained: (designed_shifts) -> reference_bond_vectors
        - designed_shifts_to_all_shifts: (designed_shifts) -> Tuple[horizontal_shifts, vertical_shifts]
        where designed_shifts is a tuple of arrays of shape (n_designed_horizontal_shifts, 2) and (n_designed_vertical_shifts, 2) defining the designed shifts.

        Args:
            default_shifts (Tuple[jnp.ndarray, jnp.ndarray]): Default shifts defining the frozen part of the geometry. Tuple of arrays of shape (n1_blocks+1, n2_blocks, 2) and (n1_blocks, n2_blocks+1, 2).
            bond_2d_id (jnp.ndarray): 2D position of the bond in the grid. Shape (2,) or (n_bonds, 2).
            vertical_bond_patch_size (jnp.ndarray): Size of the patch for vertical bonds. Shape (2,) or (n_bonds, 2).
            horizontal_bond_patch_size (jnp.ndarray): Size of the patch for horizontal bonds. Shape (2,) or (n_bonds, 2).
            bond_type (Union[Literal["vertical", "horizontal"], List[Literal["vertical", "horizontal"]]]): Type of the bond. Shape (,) or (n_bonds,).
        ...
        horizontal_shifts_mask, vertical_shifts_mask = self.compute_shifts_mask(
            bond_2d_id, vertical_bond_patch_size, horizontal_bond_patch_size, bond_type
        )

        def designed_shifts_to_all_shifts(designed_shifts):
            """
            Computes the full shifts from the designed shifts. Only the shifts in the patches are updated.

            Args:
                designed_shifts (Tuple[jnp.ndarray, jnp.ndarray]): Tuple of arrays of shape (n_designed_horizontal_shifts, 2) and (n_designed_vertical_shifts, 2) defining the designed shifts.
            """
            horizontal_shifts, vertical_shifts = default_shifts
            designed_horizontal, designed_vertical = designed_shifts
            horizontal_shifts = horizontal_shifts.at[horizontal_shifts_mask, :].set(designed_horizontal)
            vertical_shifts = vertical_shifts.at[vertical_shifts_mask, :].set(designed_vertical)
            return horizontal_shifts, vertical_shifts

        self.block_centroids_constrained = lambda designed_shifts: self.block_centroids(
            *designed_shifts_to_all_shifts(designed_shifts)
        )

        self.centroid_node_vectors_constrained = lambda designed_shifts: self.centroid_node_vectors(
            *designed_shifts_to_all_shifts(designed_shifts)
        )

        self.reference_bond_vectors_constrained = lambda designed_shifts: self.reference_bond_vectors(
            *designed_shifts_to_all_shifts(designed_shifts)
        )

        self.designed_shifts_to_all_shifts = designed_shifts_to_all_shifts
        self.shifts_mask = (horizontal_shifts_mask, vertical_shifts_mask)
        self.n_designed_horizontal_shifts = jnp.sum(horizontal_shifts_mask)
        self.n_designed_vertical_shifts = jnp.sum(vertical_shifts_mask)

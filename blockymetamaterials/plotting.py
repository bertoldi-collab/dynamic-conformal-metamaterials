...
def plot_geometry_field_overlaid(
    ...
    fig, axes = plot_geometry(
        ...
        block_displacements=data.fields[timepoint,
                                        0, :, :] if deformed else None,
        deformed=deformed,
        figsize=figsize,
        xlim=xlim,
        ylim=ylim,
    ...
    axes.collections[0].set_norm(colors.Normalize(
        vmin=field_values_min, vmax=field_values_max) if norm is None else norm)
    if colorbar:
        cb = fig.colorbar(
            axes.collections[0], pad=0.02, label=legend_label, aspect=40)
...
def generate_mode_images(data: EigenmodeData, field, out_dir, deformed=False, mode_range=None, scale_deformation=1, figsize=None, xlim=None, ylim=None, dpi=200, geometry=None, mesh=None, cmap=orange_blue_cmap(), vlim=None, legend_label=None, fontsize=14, ticksize=14, axis=True):
    ...
    for i in frames:
        ...
        axes.clear()
        axes.set_title(
            fr"$f={data.eigenvalues[i]**0.5/(2*np.pi):.4f} Hz$", fontsize=fontsize)
...
def generate_animation(data: SolutionData, field, out_filename, field_values=None, scatter_points=None, scatter_style=None, deformed=False, frame_range=None, figsize=None, xlim=None, ylim=None, fps=20, dpi=200, cmap=orange_blue_cmap(), vlim=None, legend_label=None, fontsize=14, ticksize=14, axis=True, grid=True):
    """
    docstring
    """
    ...
    # If scatter points are provided they are expected as a list of arrays of shape (n_timepoints, n_points, 2)
    if scatter_points is not None:
        scatter_plots = []
        scatter_styles = scatter_style if scatter_style is not None else [dict()]*len(scatter_points)
        for scatter_point, scatter_style in zip(scatter_points, scatter_styles):
            scatter_plots.append(
                axes.scatter(scatter_point[0, :, 0], scatter_point[0, :, 1], **scatter_style)
            )
    ...
    if data.bond_connectivity is not None:
        ...
        def animate_blocks_and_bonds(i):
            ...
            if scatter_points is not None:
                for scatter_plot, scatter_point in zip(scatter_plots, scatter_points):
                    scatter_plot.set_offsets(scatter_point[i])
            axes.set(xlim=xlim, ylim=ylim)

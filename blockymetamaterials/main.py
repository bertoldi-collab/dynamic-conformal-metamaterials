...
from blockymetamaterials.geometry import RotatedSquareGeometry, compute_inertia
from blockymetamaterials.utils import (ControlParams, GeometricalParams,
                                       LigamentParams, MechanicalParams,
                                       SolutionData, save_data)
...
inertia = compute_inertia(vertices=centroid_node_vectors(initial_angle), density=density)

# Construct energy
potential_energy = build_strain_energy(bond_connectivity=bond_connectivity(), bond_energy_fn=ligament_energy)
...
# Initial condition
state0 = jnp.array([
    0 * random.uniform(random.PRNGKey(0), (squares.n_blocks, 3)),  # Initial position
    0 * random.uniform(random.PRNGKey(1), (squares.n_blocks, 3))  # Initial velocity
])
...
print(f"Solution time (second call, i.e. using jitted solver): {time.perf_counter() - t0:.2f} s")

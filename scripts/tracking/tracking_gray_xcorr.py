...
from blockymetamaterials.utils import SolutionData, load_data, save_data
from scripts.tracking.utils import (collect_as, compute_block_displacement_from_markers, compute_block_history_from_markers,
...
def reshape_markers(marker_history):
    """Reshapes the output of the tracking code from (n_timepoints, n_markers=4*n_blocks, 2) to (n_timepoints, n_blocks, 4, 2).

    Args:
        marker_history: The positions of the markers in all frames in pixel coordinates.

    Returns:
        marker_history_reshaped: The positions of the markers in all frames in pixel coordinates in a different shape.
    """
    n_blocks = marker_history.shape[1] // 4
    marker_history_reshaped = np.zeros([marker_history.shape[0], n_blocks, 4, 2])
    for i in np.arange(n_blocks):
        marker_history_reshaped[:, i, 0, :] = marker_history[:, 4*i, :]
        marker_history_reshaped[:, i, 1, :] = marker_history[:, 4*i+1, :]
        marker_history_reshaped[:, i, 2, :] = marker_history[:, 4*i+2, :]
        marker_history_reshaped[:, i, 3, :] = marker_history[:, 4*i+3, :]

    return marker_history_reshaped

def tracking_updated(
        video_path,
        calib_xy,
        start_end_video,
        ROI_X,
    ...
    markers = np.zeros([marked_blocks.shape[0]*marked_blocks.shape[1], 2]) 

    for i in np.arange(marked_blocks.shape[0]):
        markers[4*i, :]=(marked_blocks[i, 0, :])
        markers[4*i+1, :]=(marked_blocks[i, 1, :])
        markers[4*i+2, :]=(marked_blocks[i, 2, :])
        markers[4*i+3, :]=(marked_blocks[i, 3, :])
    ...
    template_markers_blocks = np.array(markers).astype(np.float64)
    search_markers_blocks = template_markers_blocks.copy()
    current_markers_blocks = template_markers_blocks.copy()

    # Initialize the history of the markers
    markers_history = np.zeros(((endVideo - startVideo) // step_size + 1, len(markers), 2))
    ...
    while video_capture.isOpened():
        ...
        if success:
            ...
            marker_points_current = find_markers(
                    ...
                    current_frame,
                    # Used for extracting templates around the markers in the template frame
                    template_markers_blocks,
                    # Used for placing the search window around the markers in the current frame
                    search_markers_blocks,
                    search_window_size=search_window_size,
                    marker_template_size=marker_template_size,
                    upscaling_factor=upscaling_factor
                    )
            # Record the marker positions
            markers_history[(count - startVideo)//step_size] = marker_points_current
            ...
            if show_tracked_frame:
                # Draw the markers on a copy of the current frame and show it
                current_frame_markers = image.copy()
                for marker_points in marker_points_current:
                    # for marker in marker_points:
                        cv2.drawMarker(current_frame_markers, marker_points.astype(int),
                                       (0, 255, 0), cv2.MARKER_CROSS, 10, 2)
                # Show the frame and wait for key press
                cv2.imshow("Tracking", current_frame_markers)
                if cv2.waitKey(1) & 0xFF == ord('q'):
        ...
        if template_update_rate != 0 and video_capture.get(cv2.CAP_PROP_POS_FRAMES) % template_update_rate == 0:
            template_frame = current_frame.copy()
            template_markers_blocks = marker_points_current.copy()

        # Update the search window
        if search_window_update_rate != 0 and video_capture.get(cv2.CAP_PROP_POS_FRAMES) % search_window_update_rate == 0:
            search_markers_blocks = marker_points_current.copy()

    start1 = timeit.default_timer()
    print('About to reshape markers_history.')
    markers_history_reshaped = reshape_markers(markers_history)
    print("Finished reshaping. The difference of time is :", 
              timeit.default_timer() - start1)
    
    ref_markers = markers_history_reshaped[0, :, :, :]
    start2 = timeit.default_timer()
    print('About to compute block history from markers.')
    block_history = compute_block_history_from_markers(ref_markers, markers_history_reshaped, framerate, step_size, calib_xy=calib_xy, max_angle_change=max_angle_change, max_displacement_change=max_displacement_change)
    print("Finished computing block history. The difference of time is :", 
              timeit.default_timer() - start2)

    # Replace NaNs with interpolated values.
    solution = interpolate_nans(block_history)
    # solution = interpolate_nans(solution)

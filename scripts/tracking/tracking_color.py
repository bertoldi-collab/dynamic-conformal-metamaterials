"""
    This is the main script to track the deformations and rotations of the blocks during the experiment using a color image.
...
from scripts.tracking.utils import (calculate_displacement, collect_as,
                                    ...
                                    sort_contours, max_angle_change_default, conv_size_default, step_size_default, aspect_ratio_threshold_default)


def preprocessing(img, blur_size, upper, lower, morphological_transformation=morphological_transformation_default):
...
def get_contours(img, ROI_XY, blur_size, upper, lower, block_area, morphological_transformation=morphological_transformation_default):

    img = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)

    img_ROI = img[ROI_XY[1][0]: ROI_XY[1][1], ROI_XY[0][0]: ROI_XY[0][1]]

    thresh = preprocessing(img_ROI, blur_size, upper, lower, morphological_transformation=morphological_transformation)
...
def tracking(
    ...
    Args:
        ...
        lower_colorspace (tuple[int, int, int]): Lower color in hsv space.
        upper_colorspace (tuple[int, int, int]): Upper color in hsv space.
    ...
    cnts = get_contours(
        image, ROI_XY, blur_size, upper_colorspace, lower_colorspace, block_area, morphological_transformation=morphological_transformation
    )
    ...
    rect_prev = np.zeros((n_blocks, 5))  # axis: block, x, y, angle, block_id, fitting_method
...
if __name__ == "__main__":
    ...
    solution_data = tracking(
        ...
        lower_colorspace=args.lower_colorspace,
        upper_colorspace=args.upper_colorspace,

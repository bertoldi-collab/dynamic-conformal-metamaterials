...
def find_markers(template_frame, search_frame, template_markers, search_markers, search_window_size=40, marker_template_size=20, upscaling_factor=5):
    ...
    current_markers = np.zeros(search_markers.shape, dtype=np.float32)
...
def compute_block_history_from_markers(reference_markers, current_markers, framerate, step_size, calib_xy=(1, 1), max_angle_change=max_angle_change_default, max_displacement_change=None):
    """Compute the displacement and velocity of the block from the marker positions.

    Args:
        reference_markers: (384, 4, 2) The positions of the markers in the initial frame in pixel coordinates.
        current_markers: (n_timepoints, 384, 4, 2) The positions of the markers in the current frame in pixel coordinates.
        framerate: 
        calib_xy: The calibration factor for the x and y coordinates in mm/pixel.
    ...
    n_timepoints = current_markers.shape[0]
    n_blocks = current_markers.shape[1]
    block_history = np.ones([n_timepoints, 2, n_blocks, 3])
    for j in np.arange(n_timepoints):
        current_markers_ = current_markers[j, :, :, :]
        for i in np.arange(n_blocks):
            # Compute xy displacement of the centroid
            reference_centroid = polygon_centroid(reference_markers[i, :, :])
            current_centroid = polygon_centroid(current_markers_[i, :, :])
            xy_displacement = (current_centroid - reference_centroid) * np.array(calib_xy)

            # Compute angle change using the longest edge
            reference_edges = np.roll(reference_markers[i, :, :], 1, axis=0) - reference_markers[i, :, :]
            current_edges = np.roll(current_markers_[i, :, :], 1, axis=0) - current_markers_[i, :, :]

            reference_edge_lengths = compute_edge_lengths(reference_markers[i, :, :])
            current_edge_lengths = compute_edge_lengths(current_markers_[i, :, :])

            # NOTE: The angle change is weighted by the length of the edge. This makes rotation tracking more robust to noise in the marker positions.
            theta_displacement = angle_between_unit_vectors(
                reference_edges.T/reference_edge_lengths,
                current_edges.T/current_edge_lengths
            ) @ (current_edge_lengths/current_edge_lengths.sum())
            ...
            if j > 0:
                prior_block_displacement = block_history[j-1, 0,  i, :]
                if np.linalg.norm(block_displacement[:2]-prior_block_displacement[:2]) > _max_displacement_change:
                    # NOTE: If the displacement is too large for this contour, it is set to NaN. NaNs are going to be replaced by interpolated values.
                    # This is to avoid jumps in the tracking caused by a wrong contour being matched or a contour being lost for a frame.
                    block_displacement[0] = np.nan
                    block_displacement[1] = np.nan
                    ...
                    # Make sure that the angle change is not too large
                    block_displacement[2] = np.sign(block_displacement[2]) * \
                        min(np.abs(block_displacement[2]), np.abs(prior_block_displacement[2])+max_angle_change*np.pi/180)
            else:
                if np.linalg.norm(block_displacement[:2]) > _max_displacement_change:
                    # NOTE: If the displacement is too large for this contour, it is set to NaN. NaNs are going to be replaced by interpolated values.
                    # This is to avoid jumps in the tracking caused by a wrong contour being matched or a contour being lost for a frame.
            ...
            block_history[j, 0,  i, :] = block_displacement
        
    # Calculate velocity
    velocity = np.diff(block_history[:, 0, :, :], axis=0, prepend=np.zeros_like(block_history[0])) * framerate / step_size
    block_history[:, 1, :, :] = (np.roll(velocity, -1, axis=0))[:-1, :, :] # drop last entry

...
from blockymetamaterials import __version__
from blockymetamaterials.dynamics import setup_dynamic_solver
from blockymetamaterials.energy import (build_strain_energy, ligament_energy,
                                        ligament_energy_linearized,
                                        strain_energy_bond)
from blockymetamaterials.geometry import RotatedSquareGeometry
from blockymetamaterials.kinematics import _block_to_node_displacement
from blockymetamaterials.objective import compute_space_time_xcorr
from blockymetamaterials.utils import (ControlParams, GeometricalParams, LigamentParams,
                                       MechanicalParams)
...
def test_xcorr():
    ...
    delay = 3
    _, delay_trial = compute_space_time_xcorr(sp0, jnp.roll(sp0, delay, axis=1))
...
def test_tensile_test():
    ...
    def solve_tensile_test(n1_cells: int, final_strain: float, bond_energy_fn: Callable):
        ...
        # Geometry
        initial_angle = 0
        geometry = RotatedSquareGeometry(n1_cells=n1_cells, n2_cells=1, spacing=1.0)
        ...
        Jrot = 1.815**-2 / 4 * mass * geometry.spacing**2  # rotational inertia of a single lego unit
        inertia = jnp.full((geometry.n_blocks, 3), jnp.array([mass, mass, Jrot]))
        damped_blocks = jnp.arange(geometry.n_blocks)
        damping = 0.05 * jnp.full((geometry.n_blocks, 3),
                                  jnp.array([
                                      (k_stretch * mass)**0.5,
                                      (k_stretch * mass)**0.5,
                                      (k_stretch * mass)**0.5 * geometry.spacing**2 / 4
                                  ]))

        # Initial conditions
        state0 = jnp.array([
            0 * random.uniform(random.PRNGKey(0), (geometry.n_blocks, 3)),  # Initial position
            0 * random.uniform(random.PRNGKey(1), (geometry.n_blocks, 3))  # Initial velocity
        ])

        # Clamp the left end
        constrained_block_DOF_pairs = jnp.array([[0, 0], [geometry.n1_blocks, 0]])
        ...
        loaded_block_DOF_pairs = jnp.array([[geometry.n1_blocks - 1, 0], [geometry.n_blocks - 1, 0]])

        # def smooth_ramp(x, beta):
        #     return x**beta / (x**beta + (1 - x)**beta)

        def loading(state, t): return final_load * jnp.where(t < loading_rate**-1, t * loading_rate, 1)

        # Construct energy
        potential_energy = build_strain_energy(bond_connectivity=bond_connectivity(), bond_energy_fn=bond_energy_fn)
...
def test_frame_invariance_ligament_energy():
    ...
    strain_energy = strain_energy_bond(bond_connectivity=bond_connectivity, bond_energy_fn=ligament_energy)
